...
var GradingView = ValidatingView.extend({
    ...
    renderGracePeriod: function() {
        ...
        this.$el.find('#course-grading-graceperiod').val(
            StringUtils.interpolate(
                "{graceHours}:{graceMintues}",
                {
                    graceHours: format(grace_period.hours),
                    graceMintues: format(grace_period.minutes)
                }
            )
        );
    },
    ...
    renderCutoffBar: function() {
        ...
        _.each(this.descendingCutoffs,
            ...
            var newBar = this.gradeCutoffTemplate({
                ...
                removable : removable });
            HtmlUtils.append(gradelist,  newBar);
            if (draggable) {
    ...
    addNewGrade: function(e) {
        ...
        var newGradeString = this.gradeCutoffTemplate({
        ...
        gradeDom.children().last().before(newGradeString);
    ...
    setFailLabel: function() {
        HtmlUtils.setHtml(this.$el.find('.grades .letter-grade').last(), this.failLabel());
    },
    setTopGradeLabel: function() {
        HtmlUtils.setHtml(this.$el.find('.grades .letter-grade').first(), this.descendingCutoffs[0]['designation']);
    },
